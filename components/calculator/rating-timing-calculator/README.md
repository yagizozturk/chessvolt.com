...
| File | Role |
|------|------|
| `rating-timing.config.ts` | Intervals, grace settings, difficulty mapping |
...
| Field | Meaning |
|-------|---------|
| `rating` | Elo-style difficulty (e.g. 1800–2600), or from `riddleDifficultyToRating(difficulty)` |
...
Piecewise tiers: at each `rating`, solve at or under `fullTimeMs` earns the start of the timing curve for that tier. Rows must be sorted by ascending `rating`.
...
Between two rows, `fullTimeMs` is **linearly interpolated**.
...
| `defaultOpeningVariantRating` | `2000` | When variant has no difficulty field |

### Riddle difficulty → rating

```
t = (difficulty - minDifficulty) / (maxDifficulty - minDifficulty)
rating = round(minRating + t × (maxRating - minRating))
```

Defaults: difficulty 1–10 → rating 1200–2600 (from interval endpoints).
...
Rating is clamped to `[minRating, maxRating]` from the interval table.
...
- `timingRating` from `riddleDifficultyToRating(riddle.difficulty)`
...
3. Edit `riddleDifficultyAnchors` or mapping logic if difficulty scale changes.

## Exported helpers

- `getFullScoreTimeMs(rating)`
- `getGraceDurationMs(rating)`
- `riddleDifficultyToRating(difficulty)`
