...
import { lichessOpeningTagToSlug, lichessTagToSlug, slugToTitle } from "./lichess-slug";
import type {
  LichessThemeMap,
...
function resolveTagToTheme(
  tag: string,
  map: LichessThemeMap,
  sourceOrder: number,
): { slug: string; name: string; category: string; sourceOrder: number } {
  ...
  const slug = lichessTagToSlug(tag);
  return {
    slug,
    name: slugToTitle(slug),
    category: map.defaultUnknownCategory ?? "tactics",
    sourceOrder,
  };
}
...
export function resolveEducationalThemes(
  ...
  tags.forEach((tag, sourceOrder) => {
    ...
    educational.push(resolveTagToTheme(tag, map, sourceOrder));
  });
