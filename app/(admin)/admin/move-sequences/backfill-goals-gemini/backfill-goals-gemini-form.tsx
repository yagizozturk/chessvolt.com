...
import {
  backfillMoveSequenceGoalsGeminiAction,
  type BackfillGoalsGeminiFormState,
} from "@/app/(admin)/admin/move-sequences/actions/backfill-goals-gemini";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Field, FieldGroup, FieldLabel } from "@/components/ui/field";
import { Input } from "@/components/ui/input";

const initialState: BackfillGoalsGeminiFormState = { error: null, result: null };

export function BackfillGoalsGeminiForm() {
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Generate goals with Gemini</CardTitle>
          <CardDescription>
            Fetches move sequences with null goals, calls Google Gemini, and writes one goal per
            half-move. Process a small batch at a time to avoid timeouts.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <FieldGroup>
            ...
            <Field className="flex flex-row items-center gap-2">
              <input id="dryRunGemini" name="dryRun" type="checkbox" />
              <FieldLabel htmlFor="dryRunGemini" className="mb-0">
