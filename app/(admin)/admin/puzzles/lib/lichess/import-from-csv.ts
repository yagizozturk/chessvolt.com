...
export async function importLichessPuzzlesFromCsv(
  ...
  const existingSourceIds = await puzzleRepo.findExistingSourceIds(
    supabase,
    LICHESS_IMPORT_SOURCE,
  ...
  for (const row of rows) {
    ...
    const rating = parsePuzzleRating(row.rating);
    if (rating == null) {
      errors++;
      rowResults.push({
        status: "error",
    ...
    try {
      ...
      const puzzle = await puzzleRepo.create(supabase, {
      ...
      if (themeLinks.length > 0) {
        const ok = await puzzleThemeService.addPuzzleThemes(
          supabase,
          themeLinks.map((item) => ({
            puzzleId: puzzle.id,
        ...
        if (!ok) {
          throw new Error("Could not create all puzzle theme links");
        }
      }

      existingSourceIds.add(row.puzzleId);
      imported++;
      rowResults.push({ status: "imported", puzzleId: row.puzzleId, createdPuzzleId: puzzle.id });
    } catch (error) {
      errors++;
      rowResults.push({
        status: "error",
