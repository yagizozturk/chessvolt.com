...
export async function importPgnAction(formData: FormData) {
  ...
  for (const gamePgn of games) {
    ...
    if (!parsed) {
      errors.push("Could not parse or validate PGN: " + gamePgn.slice(0, 50) + "...");
      continue;
    }
    ...
    if (game) {
      ...
      const movesAtPly0 = getUciMovesFromPgnAfterPlyAtMoveCount(parsed.pgn, 0, 1);
      const defaultTitle =
  ...
  if (inserted.length > 0) {
    const query =
      errors.length > 0 ? `?imported=${inserted.length}&errors=${errors.length}` : `?imported=${inserted.length}`;
